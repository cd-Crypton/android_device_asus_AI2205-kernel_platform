...
#include <linux/soc/qcom/battery_charger_asus.h> //ASUS_BSP
#include <linux/soc/qcom/panel_event_notifier.h>
#include "fg-core.h" //ASUS_BSP
#include <linux/time.h> //ASUS_BSP
...
//ASUS_BSP +++
#define GET_PROP_CACHE_TIME 2 //get pop_cache during this time (seconds)
#define GET_PROP_CACHE_GLINK_MAX 100 //get pop_cache larger then this value (battery_chg_write count per second)

//extern int asus_chg_resume(struct device *dev);
extern bool g_vbus_plug;
extern bool power_supply_change_flag;
...
extern u64 glink_write_loading;
extern long acc_paneloff_time;
extern int bat_cur_counter;
extern bool stop_battery_soc_record_worker_flag;
//bool battery_chg_probe_complete = 0; 
//ASUS_BSP ---
...
static void battery_chg_check_status_work(struct work_struct *work)
{
	 ...
	 */
	//pr_err("battery_chg_check_status_work5++\n");
	...
	//pr_err("battery_chg_check_status_work6++\n");
				...
				size_t len)
{
	...
	pr_debug("qti_handle_notification++\n");
	...
	switch (notification) {
	case BC_BATTERY_STATUS_GET:	
		pr_err("BC_BATTERY_STATUS_GET ++\n");
		if (g_bcdev) 	
		//check init
		{	
			pst = &bcdev->psy_list[PSY_TYPE_BATTERY];
			pr_err("BC_BATTERY_STATUS_GET:power_supply_changed\n");
		...
		union power_supply_propval *pval)
{
	...
	int prop_id, rc;
	//ASUS_BSP +++
	static int prop_cache[WLS_PROP_MAX] = {-1};
	static struct timespec64  prop_lasttime[WLS_PROP_MAX];
	struct timespec64  mtNow;
	//ASUS_BSP ---

	pval->intval = -ENODATA;
	ktime_get_coarse_real_ts64(&mtNow); //ASUS_BSP
	
	prop_id = get_property_id(pst, prop);
	if (prop_id < 0)
		return prop_id;

	//ASUS_BSP +++ get from prop_chache
	if (((mtNow.tv_sec - prop_lasttime[prop_id].tv_sec <= GET_PROP_CACHE_TIME) || glink_write_loading >= GET_PROP_CACHE_GLINK_MAX) && (prop_cache[prop_id]!=-1)) {
		pval->intval = prop_cache[prop_id];
		//pr_err("Use cache prop_cache[%d]=%d",prop_id,pst->prop[prop_id]);
		return 0;
	}
	//ASUS_BSP ---
	...
	ktime_get_coarse_real_ts64(&prop_lasttime[prop_id]);	
	//pr_err("prop_id[%d]=%d",prop_id,pst->prop[prop_id]);
	//ASUS_BSP ---
...
static int usb_psy_set_icl(struct battery_chg_dev *bcdev, u32 prop_id, int val)
{
	...
	u32 temp;
	int rc;
	static int pre_val=-1; //ASUS_BSP
    
	//ASUS_BSP +++
	if(pre_val == val){
        pr_err("skip double to set ICL\n");
		return 0;
	}
	pre_val = val;
	//ASUS_BSP ---
		...
		union power_supply_propval *pval)
{
	...
	int prop_id, rc;
	//ASUS_BSP +++
	static int prop_cache[USB_PROP_MAX] = {-1};
	static struct timespec64  prop_lasttime[USB_PROP_MAX];
	struct timespec64  mtNow;
	//ASUS_BSP ---
	...
	//ASUS_BSP +++ get from prop_chache
	if ((((mtNow.tv_sec - prop_lasttime[prop_id].tv_sec <= GET_PROP_CACHE_TIME) || glink_write_loading >= GET_PROP_CACHE_GLINK_MAX) && (prop_cache[prop_id]!=-1)) &&
		(prop_id != USB_ONLINE) && (prop_id != USB_TYPE) && (prop_id != USB_ADAP_TYPE)){
		pval->intval = prop_cache[prop_id];
		//pr_err("Use cache prop_cache[%d]=%d",prop_id,pst->prop[prop_id]);
		return 0;
	}
	...
	if (prop == POWER_SUPPLY_PROP_ONLINE && pval->intval == 0) {	// ASUS BSP: set online = 1 when feature usbin_suspend with cable
		if (g_vbus_plug){
			pr_err("[BAT][CHG] USB_ONLINE=0, but vbus=%d", g_vbus_plug);
			pval->intval = 1;
		}
	}
		...
		union power_supply_propval *pval)
{
	...
	int prop_id, rc;
//ASUS_BSP +++
	static int prop_cache[BATT_PROP_MAX] = {-1};
	static struct timespec64  prop_lasttime[BATT_PROP_MAX];
	struct timespec64  mtNow;
	int Enable_Bypass_BATT_CAPACITY_Cache = 1;
...
//ASUS_BSP remove
	...
	//BATT_CAPACITY use cache, don't bypass
	if (stop_battery_soc_record_worker_flag==1)
		Enable_Bypass_BATT_CAPACITY_Cache=0;
		
	if (Enable_Bypass_BATT_CAPACITY_Cache ==1)
	{
		//get from prop_chache
		if ((((mtNow.tv_sec - prop_lasttime[prop_id].tv_sec <= GET_PROP_CACHE_TIME) || glink_write_loading >= GET_PROP_CACHE_GLINK_MAX) 
		&& (prop_cache[prop_id]!=-1)) && (prop_id != BATT_STATUS) && (prop_id != BATT_CHG_TYPE) && (prop_id != BATT_CURR_NOW) 
		&& (prop_id != BATT_VOLT_NOW) && (prop_id != BATT_CAPACITY)){
			pval->intval = prop_cache[prop_id];
			//pr_err("Use cache prop_cache[%d]=%d",prop_id,pst->prop[prop_id]);
		return 0;
		}
	}
	else if (Enable_Bypass_BATT_CAPACITY_Cache == 0)
	{
		if ((((mtNow.tv_sec - prop_lasttime[prop_id].tv_sec <= GET_PROP_CACHE_TIME) || glink_write_loading >= GET_PROP_CACHE_GLINK_MAX) 
		&& (prop_cache[prop_id]!=-1)) && (prop_id != BATT_STATUS) && (prop_id != BATT_CHG_TYPE) && (prop_id != BATT_CURR_NOW) 
		&& (prop_id != BATT_VOLT_NOW)){
			pval->intval = prop_cache[prop_id];
			//pr_err("Use cache prop_cache[%d]=%d",prop_id,pst->prop[prop_id]);
			return 0;
		}
...
//ASUS_BSP ---
	...
	case POWER_SUPPLY_PROP_TEMP:
		...
		//battery-65-shutdown ++ 
		if (pval->intval >= 650)  //650	
			{					
				//CHG_DBG_E("[BAT][CHG]%s POWER_SUPPLY_PROP_TEMP = %d", __func__,pval->intval);
				//pr_err("[BAT][CHG]%s POWER_SUPPLY_PROP_TEMP = %d", __func__,pval->intval);		
				//CHG_DBG_E("%s: POWER_SUPPLY_PROP_TEMP = %d \n", __func__,temp);
				//CHG_DBG_E("[BAT][CHG]%s Initiating a shutdown in 100 ms", __func__);
				//pr_err("[BAT][CHG]%s Initiating a shutdown in 100 ms\n", __func__);	
				//CHG_DBG_E("%s: Initiating a shutdown in 100 ms\n");
				//msleep(100);
				//CHG_DBG_E("[BAT][CHG]%s Attempting kernel_power_off: Battery temp too high", __func__);
				//pr_err("[BAT][CHG]%s Attempting kernel_power_off: Battery temp too high\n", __func__);				
				//IMS already add this feature in AI2205 => disable it in BSP part
				pr_err("[BAT][CHG]%s kernel detects battery temp = %d too high\n", __func__,pval->intval);
				//kernel_power_off();	
			}
		//battery-65-shutdown --	
...
//ASUS_BSP +++
	case POWER_SUPPLY_PROP_STATUS:
		pval->intval = pst->prop[prop_id];
		if (g_vbus_plug == 0 && pval->intval == 1) {
			pval->intval = 2;
		}
		if (g_vbus_plug && pval->intval == 2) {
			pval->intval = 1;
		}
	...
	case POWER_SUPPLY_PROP_HEALTH:  
		...
		//health_type[bat_health],  Over voltage = 4  => bat_health=4;
		if ((qti_cell1_voltage < 3300 || qti_cell2_voltage < 3300) 
		&& (qti_cell1_voltage != 3000 && qti_cell2_voltage != 3000))
		{							
			pval->intval = 4;   	
...
#endif	
//ASUS_BSP ---
	if(prop_id != BATT_MODEL_NAME){ //ignore model name due to it is strng not integer type
		prop_cache[prop_id] = pval->intval;
	}
	ktime_get_coarse_real_ts64(&prop_lasttime[prop_id]);
	//pr_err("prop_id[%d]=%d",prop_id,pst->prop[prop_id]);
...
//	&class_attr_wireless_type.attr, //ASUS_BSP -
...
pr_err("opcode=0x%x call battery_chg_write ", msg_notify.hdr.opcode);
...
extern int g_drm_blank;
extern struct delayed_work asus_set_panelonoff_current_work;
extern struct delayed_work asus_set_panelonoff_Gauge_EN_work;
			...
			struct panel_event_notification *notification, void *data)
{
	...
	pr_err("panel event received, type: %d\n", notification->notif_type);
	...
	case DRM_PANEL_EVENT_BLANK:
		...
		schedule_delayed_work(&asus_set_panelonoff_Gauge_EN_work, 0);  //display off
		break;
	case DRM_PANEL_EVENT_UNBLANK:
		battery_chg_notify_enable(bcdev);
		schedule_delayed_work(&asus_set_panelonoff_current_work, 0);
		cancel_delayed_work(&asus_set_panelonoff_Gauge_EN_work); //display on
		bat_cur_counter = 0 ;
		break;
	default:
...
static int battery_chg_probe(struct platform_device *pdev)
{
	...
	bcdev->initialized = true; //20230508
	...
	bcdev->notify_en = false;
	battery_chg_notify_enable(bcdev);
	//device_init_wakeup(bcdev->dev, true); //porting -
...
static int battery_chg_remove(struct platform_device *pdev)
{
	...
	//device_init_wakeup(bcdev->dev, false); //porting -
