...
struct ssd1362_platform_data {
	...
	int atd_pattern;
	int frame_size;
	struct delayed_work boot_logo_work;
	...
	if (pwr_on) {
		...
		if (pdata->power_disabled) {
			...
			if (pdata->panel_id) {
				...
				pr_debug("[PMOLED] VCI H\n");
				gpio_direction_output(pdata->vci_gpio, 1);
				mdelay(2);
				pr_debug("[PMOLED] PWR en H\n");
				gpio_direction_output(pdata->pwren_gpio, 1);
				mdelay(10); //Toronto spec request
				pr_debug("[PMOLED] reset H\n");
...
{
	if (cmd) {
		pr_debug("[PMOLED] DC L\n");
		gpio_direction_output(pdata->dc_gpio, 0);
	} else {
		pr_debug("[PMOLED] DC H\n");
...
static int ssd1362_gpio_get_value(int gpio, struct ssd1362_platform_data *pdata)
{
	int value = -1;
	int err = -1;

	if (gpio <= 0 || pdata->pins_active == NULL || pdata->pinctrl == NULL) {
		pr_err("[PMOLED] gpio or pin not active\n");
		return -1;
	}

	err = pinctrl_select_state(pdata->pinctrl, pdata->pins_active);
	if (err)
		pr_err("pinctrl_select_state err:%d\n", __func__, err);
	gpio_direction_output(gpio, 1);
	msleep(1);
	gpio_direction_input(gpio);
	value = gpio_get_value(gpio);
	pr_err("[PMOLED] reading GPIO %d = %d\n", gpio, value);
	return value;
}
...
static int ssd1362_parse_dt(struct device *dev, struct ssd1362_platform_data *pdata)
{
	...
	int id_gpio = 0;
	int lgf_prj_1_gpio;
	u32 lgf_prj_1_gpio_flag;
	int lgf_prj_2_gpio;
	u32 lgf_prj_2_gpio_flag;
	int lgf_prj_3_gpio;
	u32 lgf_prj_3_gpio_flag;
	...
	pr_err("[PMOLED] GPIO, ID: %d, RESET: %d, DC: %d, PWREN: %d, VCI: %d\n",
				id_gpio, pdata->reset_gpio, pdata->dc_gpio, pdata->pwren_gpio, pdata->vci_gpio);
	...
	if (id_gpio > 0) {
		pdata->panel_id = ssd1362_gpio_get_value(id_gpio, pdata);
		pr_err("[PMOLED] panel id = %d\n", pdata->panel_id);
	} else {
		pdata->panel_id = 1;
		pr_err("[PMOLED] preSR and later, set panel id = %d\n", pdata->panel_id);
	}

	// Get if PMOLED is attached by reading project ID
	lgf_prj_1_gpio = of_get_named_gpio_flags(np, "asus,light-guide-film-id1", 0, &lgf_prj_1_gpio_flag);
	lgf_prj_2_gpio = of_get_named_gpio_flags(np, "asus,light-guide-film-id2", 0, &lgf_prj_2_gpio_flag);
	lgf_prj_3_gpio = of_get_named_gpio_flags(np, "asus,light-guide-film-id3", 0, &lgf_prj_3_gpio_flag);
	if (lgf_prj_1_gpio > 0 && lgf_prj_2_gpio > 0 && lgf_prj_3_gpio > 0) {
		int prj_1 = ssd1362_gpio_get_value(lgf_prj_1_gpio, pdata);
		int prj_2 = ssd1362_gpio_get_value(lgf_prj_2_gpio, pdata);
		int prj_3 = ssd1362_gpio_get_value(lgf_prj_3_gpio, pdata);
		printk("[PMOLED] LGF PRJ gpio specified, prj1,2,3 = %d,%d,%d\n", prj_1, prj_2, prj_3);
		if (prj_1 > 0)
			pdata->have_panel = 0;
		else
			pdata->have_panel = 1;
	} else {
		printk("[PMOLED] LGF PRJ gpio not specified, treat as presented.\n");
		pdata->have_panel = 1;
	}
...
{
	pr_err("[PMOLED] show logo\n");
	if (pdata->panel_id) {
		ssd1362_panel_on(pdata, false, pdata->panel_id);
					 ...
					 size_t count)
{
	...
	for (i=0; i<frame_num; i++) {
		...
		snprintf(path, 255, "/data/pmoled/pic%03d.bmp", i);
					 ...
					 size_t count)
{
	...
	if (val<0 || val>2) {
		...
		set_fs(KERNEL_DS);
		memset(path, 0x00, sizeof(path));
		snprintf(path, 255, "/data/pmoled_dump%d.raw", val);
...
static void ssd1362_boot_logo_work_func(struct work_struct *work)
{
	struct ssd1362_platform_data *pdata =
		container_of(to_delayed_work(work), struct ssd1362_platform_data, boot_logo_work);

	show_logo(pdata);
}

static void ssd1362_gpio_check_work_func(struct work_struct *work)
{
	int value = -1;
	static int ticks = 300; //300*100 = 30 seconds
	struct ssd1362_platform_data *pdata =
		container_of(to_delayed_work(work), struct ssd1362_platform_data, gpio_check_work);

	//do check gpio here
	value = gpio_get_value(pdata->pwren_gpio);

	if (value == 0) {
		pr_err("[PMOLED] gpio 93 has been pulled down, reopen it");
		schedule_delayed_work(&pdata->boot_logo_work, msecs_to_jiffies(100));
	} else if (ticks -- > 0) {
		schedule_delayed_work(&pdata->gpio_check_work, msecs_to_jiffies(100));
	} else {
		pr_err("[PMOLED] gpio check done, no issue found\n");
	}
}
...
static int ssd1362_probe(struct spi_device *spi)
{
...
#else
	if (pdata->have_panel == 0) {
		pr_err("[PMOLED] no pmoled, return -ENODEV;\n");
		gpio_direction_input(pdata->reset_gpio);
		gpio_direction_input(pdata->vci_gpio);
		gpio_direction_input(pdata->pwren_gpio);
		gpio_direction_input(pdata->dc_gpio);
		devm_kfree(&spi->dev, pdata);
		return -ENODEV;
	}
	...
	// use gpio check work to check if out gpio 93 been pulled down
	INIT_DELAYED_WORK(&pdata->boot_logo_work, ssd1362_boot_logo_work_func);
	INIT_DELAYED_WORK(&pdata->gpio_check_work, ssd1362_gpio_check_work_func);
	//schedule_delayed_work(&pdata->gpio_check_work, msecs_to_jiffies(100));
