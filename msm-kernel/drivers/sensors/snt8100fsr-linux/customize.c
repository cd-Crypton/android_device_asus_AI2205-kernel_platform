...
void grip_input_event_report(int g_id, int len, int trk_id, int bar_id, int force, int fr_nr, int center, int top, int bottom){
	...
	if(snt8100fsr_g->en_sensor_evt){
		...
		if (gs_found ||(tr[i].bar_id==0 && tr[i].trk_id==0 && tr[i].force_lvl > TR_DIAG_REC_VERS_MAX)) {
			if (gs_found == 0){
				...
				if(p->tap > 0){
					if((p->tap & 0x10)>0){
						...
						grip_input_event_report(gesture_id, count, 1, tr[i].bar_id, tr[i].force_lvl, frame, tr[i].center, tr[i].top, tr[i].bottom);
					...
					if((p->tap & 0x1)>0){
						gesture_id = GRIP_TAP_L1;
						grip_input_event_report(gesture_id, count, 0, tr[i].bar_id, tr[i].force_lvl, frame, tr[i].center, tr[i].top, tr[i].bottom);
				...
				if(gesture_id == GRIP_SWIPE_L){
					if(p->swipe0_velocity > 0) //bottom to top
						grip_input_event_report(gesture_id, 0, 0, 0 ,p->swipe0_velocity, 0, 0, 0, 0);
					else//top to bottom
						grip_input_event_report(gesture_id, 0, 1, 0 ,p->swipe0_velocity, 0, 0, 0, 0);
				}else if(gesture_id == GRIP_SWIPE_R){
					if(p->swipe1_velocity > 0) //bottom to top
						grip_input_event_report(gesture_id, 0, 0, 0 ,p->swipe1_velocity, 0, 0, 0, 0);
					else//top to bottom
						grip_input_event_report(gesture_id, 0, 1, 0 ,p->swipe1_velocity, 0, 0, 0, 0);
				}else{
				}
				if(gesture_id!=0)
				...
				if (p->escape0 == GS_RPT_TYPE_SLIDE) {
					...
					if(fid0 == 1){
						if (id0 == 0) {
							...
							grip_input_event_report(gesture_id, count, fid0, id0, p->slider_force0, frame, p->slider_pos0, tr[i].top, tr[i].bottom);
					...
					if(fid1 == 1){
						if(id1==1){
							...
							PRINT_INFO("3. Slider[%u,%u]: F%u, P%u, g_id=%d", id1, fid1,
								...
								gesture_id);
							grip_input_event_report(gesture_id, count, fid1, id1, p->slider_force1, frame, p->slider_pos1, tr[i].top, tr[i].bottom);
					...
					*/
					...
					if(p_sq->squeeze[0] > 0){
						...
						grip_input_event_report(gesture_id, count,  sq2_gesture, tr[i].bar_id, tr[i].force_lvl, frame, tr[i].center, tr[i].top, tr[i].bottom);
					}
					if(p_sq->squeeze[1] > 0){
						gesture_id = 0;
						sq2_gesture = 1;
			...
			if(grip_game_gesture_status()){
				//Normal Data
				grip_input_event_report(0, count, tr[i].trk_id, tr[i].bar_id, tr[i].force_lvl, frame, tr[i].center, tr[i].top, tr[i].bottom);
