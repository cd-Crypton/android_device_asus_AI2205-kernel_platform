...
void upload_wq_func(struct work_struct *work_orig) {
	...
	static int hw_id_status_count = 0;
	...
	*/
	...
	while(snt8100fsr_g->selected_fw_version == -1 && null_count < GRIP_WAIT_FW_PROBE){
		null_count++;
		msleep(1000);
	}
	if(snt8100fsr_g->selected_fw_version == -1 && null_count >= GRIP_WAIT_FW_PROBE){
		PRINT_ERR("Failed to read selected_fw_version, use default firmware version");
...
*/
	...
	PRINT_INFO("cur_total_read=%d, fw_size=%d", cur_total_read, fwsize);
	snt8100fsr_g->chip_reset_flag = GRIP_FW_DL_END;
...
static void upload_firmware_internal() {
	...
	while(retry_count > 0){
		...
		if(snt8100fsr_g->selected_fw_version == 0)
			ret = request_firmware(&fw, REQUEST_GRIP_FW_PATH_V0, snt8100fsr_g->dev);		
		else if(snt8100fsr_g->selected_fw_version == 1)
			ret = request_firmware(&fw, REQUEST_GRIP_FW_PATH_V1, snt8100fsr_g->dev);
		else if(snt8100fsr_g->selected_fw_version == 2)
			ret = request_firmware(&fw, REQUEST_GRIP_FW_PATH_V2, snt8100fsr_g->dev);
		else
			ret = request_firmware(&fw, REQUEST_GRIP_FW_PATH_V3, snt8100fsr_g->dev);		
