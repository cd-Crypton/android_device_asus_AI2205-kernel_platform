...
ssize_t sysfs_chip_reset_store(struct device *dev,
	...
	if(snt8100fsr_g->selected_fw_version == 0)
			err = request_firmware(&fw, REQUEST_GRIP_FW_PATH_V0, dev);		
	else if(snt8100fsr_g->selected_fw_version == 1)
			err = request_firmware(&fw, REQUEST_GRIP_FW_PATH_V1, dev);
	else if(snt8100fsr_g->selected_fw_version == 2)
			err = request_firmware(&fw, REQUEST_GRIP_FW_PATH_V2, dev);
	else
			err = request_firmware(&fw, REQUEST_GRIP_FW_PATH_V3, dev);	
...
static struct device_attribute grip_property_attrs[] = {
	...
	__ATTR(log_track_report, 0664, NULL, sysfs_log_track_report_store),
	...
	__ATTR(log_d1test, 0664, NULL, sysfs_log_d1test_store),
	__ATTR(log_frames, 0664, NULL, sysfs_log_frames_store),
