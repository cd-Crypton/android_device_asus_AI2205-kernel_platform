...
void snt_set_pinctrl(struct device *dev, char *str)
{
	...
	if(IS_ERR(set_state)){
		PRINT_ERR("%s: pinctrl_lookup_state (%s)ERROR.\n", __FUNCTION__, str);
		return;
	}
...
*/
	PRINT_INFO("id pin request");
	gpio_req = of_get_named_gpio(snt8100fsr_g->dev->of_node, SNT_ID_NAME, 0);
	gpio_request(gpio_req, "snt-id-gpio");

	ret = gpio_direction_input(gpio_req);
	if (ret < 0) {
		PRINT_ERR("%s: gpio_direction_input ERROR(%d). \n", __FUNCTION__, ret);
		return ret;
	}
	snt_set_pinctrl(snt8100fsr_g->dev, GRIP_SNT_ID_ACTIVE);
	msleep(1);
	snt8100fsr_g->fc_id_status = gpio_get_value(gpio_req);
	PRINT_INFO("ID pin pull-down");
	snt_set_pinctrl(snt8100fsr_g->dev, GRIP_SNT_ID_SUSPEND);
	...
	PRINT_INFO("rst pin pull-up done, rst=%d, id=%d", 
		   gpio_get_value(snt8100fsr_g->rst_gpio), snt8100fsr_g->fc_id_status);
...
void asus_init_probe(void){
	...
	grip_status_g->G_SLIDE_SENSE_EN = 0;
	grip_status_g->G_BENDING_EN = 1;
	...
	create_Grip_Slide_Sense_En_proc_file();	
	...
	create_Grip_selected_fw_version_proc_file();
	
	/* bending */
	create_Grip_bending_proc_file();
}
