...
#define ASUS_GRIP_SENSOR_FRAME_DATE_SIZE        10240
#define ASUS_GRIP_SENSOR_D1TEST_DATA_REPORT_SIZE       7840
#define ASUS_GRIP_SENSOR_TRACK_REPORT_DATA_SIZE	       2048
...
//For Sentons Factory Tool
#define ASUS_GRIP_SENSOR_IOCTL_SET_LOG_FRAMES           _IOW(ASUS_GRIP_SENSOR_IOC_MAGIC, 24, int)	///< Grip sensor ioctl command - SET_LOG_FRAMES
#define ASUS_GRIP_SENSOR_IOCTL_GET_LOG_FRAMES           _IOR(ASUS_GRIP_SENSOR_IOC_MAGIC, 25, unsigned char[ASUS_GRIP_SENSOR_FRAME_DATE_SIZE])	///< Grip sensor ioctl command - GET_LOG_FRAMES DATA per frame
#define ASUS_GRIP_SENSOR_IOCTL_SET_LOG_D1TEST           	_IOW(ASUS_GRIP_SENSOR_IOC_MAGIC, 26, int)       ///< Grip sensor ioctl command - SET_LOG_D1TEST
#define ASUS_GRIP_SENSOR_IOCTL_GET_LOG_D1TEST           	_IOR(ASUS_GRIP_SENSOR_IOC_MAGIC, 27, unsigned char[ASUS_GRIP_SENSOR_D1TEST_DATA_REPORT_SIZE])   ///< Grip sensor ioctl command - GET_LOG_D1TEST DATA per frame
#define ASUS_GRIP_SENSOR_IOCTL_ENABLE_LOG_TRACK_REPORT          _IOW(ASUS_GRIP_SENSOR_IOC_MAGIC, 28, int)       ///< Grip sensor ioctl command - ON/OFF LOG_TRACK_REPORT
#define ASUS_GRIP_SENSOR_IOCTL_GET_LOG_TRACK_REPORT           	_IOR(ASUS_GRIP_SENSOR_IOC_MAGIC, 29, char[ASUS_GRIP_SENSOR_TRACK_REPORT_DATA_SIZE])   ///< Grip sensor ioctl command - GET_LOG_TRACK_REPORT data
...
static long SntSensor_miscIoctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	//unsigned char frame_data[5120];
	/* fw data load */
	unsigned char fw_data[512];
	static int fw_data_size = 0;
        static int frame_cap_num = 0;
        static int d1test_cap_num = 0;
	static int enable = 0;
	...
	switch (cmd) {
		...
		case ASUS_GRIP_SENSOR_IOCTL_ID_STATE:
			...
			break;
		// Sentons Stanly add the log_frame set and get
                case ASUS_GRIP_SENSOR_IOCTL_SET_LOG_FRAMES:
                        ret = copy_from_user(&frame_cap_num, (int __user*)arg, sizeof(frame_cap_num));
			PRINT_FUNC("frame_cap_num = %zu ", frame_cap_num);
			...
			if (frame_cap_num == 0) {
			    ...
			    if (frame_cap_num == -1)
				frame_cap_num = 0x7FFFFFFF;

			    ret = enable_frame_logging(snt8100fsr_g, (uint32_t)frame_cap_num);
			    if (ret) {
				    PRINT_CRIT("Enable frame logging failed: %d", ret);
			    }
			}

                        PRINT_DEBUG("ASUS_GRIP_SENSOR_IOCTL_SET_LOG_FRAMES \n");
                        break;
                case ASUS_GRIP_SENSOR_IOCTL_GET_LOG_FRAMES:
			while(snt8100fsr_g->active_sc_cmd == mc_frame_dump){
                        	PRINT_DEBUG("Frame data not ready \n");
				msleep(20);
			}
                        ret = copy_to_user((uint8_t  __user*)arg, frame_report, ASUS_GRIP_SENSOR_FRAME_DATE_SIZE);
                        PRINT_DEBUG("ASUS_GRIP_SENSOR_IOCTL_GET_LOG_FRAMES Done\n");
                        break;
                case ASUS_GRIP_SENSOR_IOCTL_SET_LOG_D1TEST:
                        PRINT_DEBUG("ASUS_GRIP_SENSOR_IOCTL_SET_LOG_D1TEST \n");
                        ret = copy_from_user(&d1test_cap_num, (int __user*)arg, sizeof(d1test_cap_num));
			#ifdef DYNAMIC_PWR_CTL
        		if (snt_activity_request() != 0) {
                		PRINT_CRIT("snt_activity_request() failed");
                		return -1;
                	...
                	PRINT_DEBUG("d1test frames = %d\n", d1test_cap_num);
                	if (d1test_cap_num == 1) {
                        	d1test_cap_num = 64 * 1024;
                	}
                	ret = enable_d1test_logging(snt8100fsr_g, (uint32_t)d1test_cap_num);
			if (ret) {
				PRINT_CRIT("Enable d1test logging failed: %d", ret);
			}
			break;
                case ASUS_GRIP_SENSOR_IOCTL_GET_LOG_D1TEST:
			while(snt8100fsr_g->active_sc_cmd == mc_d1_test){
                        	PRINT_DEBUG("D1test data not ready \n");
				msleep(20);
			}
                        ret = copy_to_user((uint8_t  __user*)arg, d1test_report, ASUS_GRIP_SENSOR_D1TEST_DATA_REPORT_SIZE);
                        PRINT_DEBUG("ASUS_GRIP_SENSOR_IOCTL_GET_LOG_D1TEST Done\n");
			break;
		case ASUS_GRIP_SENSOR_IOCTL_ENABLE_LOG_TRACK_REPORT:
                        PRINT_DEBUG("ASUS_GRIP_SENSOR_IOCTL_ENABLE_LOG_TRACK_REPORT \n");
                        ret = copy_from_user(&enable, (int __user*)arg, sizeof(enable));
        		#ifdef DYNAMIC_PWR_CTL
        		if (snt_activity_request() != 0) {
                		PRINT_CRIT("snt_activity_request() failed");
                		return -1;
			...
			// enable the track_report.txt file format
   			if (enable == 0) {
                		ret = enable_track_report_logging(false, 0);
                		if (ret) {
                        		PRINT_CRIT("Unable to disable track report logging: %d", ret);
                        		return -1;
                		}
        		} else if (enable == 1) {
                		ret = enable_track_report_logging(true, 0);
                		if (ret) {
                        		PRINT_CRIT("Unable to enable track report logging: %d", ret);
                        		return -1;
                		}
        		} else {
                		PRINT_NOTICE("Invalid Value: %d", enable);
                		return -1;
        		}
			break;
		case ASUS_GRIP_SENSOR_IOCTL_GET_LOG_TRACK_REPORT:
                        ret = copy_to_user((char  __user*)arg, track_report_buffer, ASUS_GRIP_SENSOR_TRACK_REPORT_DATA_SIZE);
                        PRINT_DEBUG("ASUS_GRIP_SENSOR_IOCTL_GET_LOG_TRACK_REPORT Done\n");
			break;
		default:
			ret = -1;
...
static int Grip_selected_fw_version_proc_read(struct seq_file *buf, void *v)
{
	seq_printf(buf, "selected fw version = %d\n", snt8100fsr_g->selected_fw_version);
	return 0;
}

static ssize_t Grip_selected_fw_version_proc_write(struct file *filp, const char __user *buff,
		size_t len, loff_t *data)
{
	int val=0;
	char messages[256];
	...
	if (strcmp("0000306", messages) == 0)
		snt8100fsr_g->selected_fw_version = 0;
	else if (strcmp("0000312", messages) == 0)
		snt8100fsr_g->selected_fw_version = 1;
	else if (strcmp("0000319", messages) == 0)
		snt8100fsr_g->selected_fw_version = 2;
	else
		snt8100fsr_g->selected_fw_version = 3;
		
	val = snt8100fsr_g->selected_fw_version;			
				
	PRINT_INFO("selected fw version = %d", val);
	return len;
}

static int Grip_selected_fw_version_proc_open(struct inode *inode, struct file *file)
{
	return single_open(file, Grip_selected_fw_version_proc_read, NULL);
}

void create_Grip_selected_fw_version_proc_file(void)
{
	static const struct proc_ops proc_fops = {
		.proc_open	= Grip_selected_fw_version_proc_open,
		.proc_read	= seq_read,
		.proc_lseek	= seq_lseek,
		.proc_write	= Grip_selected_fw_version_proc_write,
		.proc_release	= single_release,
	};
	struct proc_dir_entry *proc_file = 
	proc_create("driver/grip_select_fw_ver", 0666, NULL, &proc_fops);
...
static int Grip_bending_proc_read(struct seq_file *buf, void *v)
{
	seq_printf(buf, "%d\n", grip_status_g->G_BENDING_EN);
	return 0;
}
static ssize_t Grip_bending_proc_write(struct file *filp, const char __user *buff,
		size_t len, loff_t *data)
{
	int val;
	char messages[256];
	...
	if(grip_status_g->G_BENDING_EN==val){
		PRINT_INFO("repeat, skip it");
		return len;
	}
	
	PRINT_INFO("Grip_bending_proc_write:%d", val);
	grip_bending_func(val);
	
	return len;
}

static int Grip_bending_proc_open(struct inode *inode, struct file *file)
{
	return single_open(file, Grip_bending_proc_read, NULL);
}

void create_Grip_bending_proc_file(void)
{
	static const struct proc_ops proc_fops = {
		.proc_open	= Grip_bending_proc_open,
		.proc_read	= seq_read,
		.proc_lseek	= seq_lseek,
		.proc_write	= Grip_bending_proc_write,
		.proc_release	= single_release,
	};

	struct proc_dir_entry *proc_file = proc_create("driver/grip_bending", 0666, NULL, &proc_fops);
...
int Grip_Slide_Sense_En_proc_read(struct seq_file *buf, void *v)
{
	seq_printf(buf, "%d\n", grip_status_g->G_SLIDE_SENSE_EN);
	return 0;
}

ssize_t Grip_Slide_Sense_En_proc_write(struct file *filp, const char __user *buff,
		size_t len, loff_t *data)
{
	int val;
	char messages[256];
	...
	val = (int)simple_strtol(messages, NULL, 10);
	if(grip_status_g->G_SLIDE_SENSE_EN ==val){
		PRINT_INFO("repeat, skip it");
		return len;
	}
	grip_status_g->G_SLIDE_SENSE_EN =val;
	grip_slide_sense_switch_func(grip_status_g->G_SLIDE_SENSE_EN);
	PRINT_INFO("Grip_Slide_Sense_En_proc_write:%d", val);	
	return len;
}

int Grip_Slide_Sense_En_proc_open(struct inode *inode, struct file *file)
{
	return single_open(file, Grip_Slide_Sense_En_proc_read, NULL);
}

void create_Grip_Slide_Sense_En_proc_file(void)
{
	static const struct proc_ops proc_fops = {
		.proc_open	= Grip_Slide_Sense_En_proc_open,
		.proc_read	= seq_read,
		.proc_lseek	= seq_lseek,
		.proc_write	= Grip_Slide_Sense_En_proc_write,
		.proc_release	= single_release,
	};
	/*
	static const struct file_operations proc_fops = {
		.owner = THIS_MODULE,
		.open = Grip_Slide_Sense_En_proc_open,
		.write = Grip_Slide_Sense_En_proc_write,
		.read = seq_read,
		.release = single_release,
	};*/
	struct proc_dir_entry *proc_file = proc_create("driver/grip_slide_sense_en", 0666, NULL, &proc_fops);
