...
#else
	PRINT_INFO("DPC is off, don't into deepsleep");
#endif
}
...
void grip_slide_sense_switch_func(int val){
...
void grip_tap_enable_func(int tap_id, int val, uint16_t* reg ) {
	const char *merge_off = "128 1\n";
	const char *merge_on = "128 0\n"; 	 
	MUTEX_LOCK(&snt8100fsr_g->ap_lock);
	PRINT_DEBUG("val = %d", val);
	...
	if(grip_status_g->G_EN == 1){
		...
		*reg = (*reg & 0xFFFE) | ( val & 0x0001);
		...
		if(val == 1){
			...
			if(snt8100fsr_g->selected_fw_version == 3){
				msleep(50);
				grip_set_sys_param(merge_off);
			}								
		}else{
			grip_set_game_gesture_sysp(false);
			if( (!grip_tap_gesture_status()) && (snt8100fsr_g->selected_fw_version == 3) )
			{
				msleep(50);
				grip_set_sys_param(merge_on);
			}			
		}
...
void grip_tapX_enable_func(int val, int id, uint16_t *reg_val ){
	...
	grip_bending_func(grip_status_g->G_BENDING_EN);		
...
void grip_bending_func(int val){
	int ret;
	uint16_t RegWrite_t = 0;
	if(grip_status_g->G_EN <= 0){
		PRINT_INFO("Skip setting when grip off");
		return;
	}
	...
	if(grip_status_g->G_BENDING_EN == val){
		mutex_unlock(&snt8100fsr_g->ap_lock);
		PRINT_INFO("Skip grip_bending the same setting");
		return;
	}
	grip_status_g->G_BENDING_EN = val;
	if(Grip_Check_FW_Status()){ return; }
	...
	ret = write_register(snt8100fsr_g, REGISTER_BENDING, &RegWrite_t);
	if(ret < 0) {
		PRINT_ERR("Write reg 0x%X fail", REGISTER_BENDING);	
	}
	msleep(50);
	PRINT_INFO("BENDING=%d", grip_status_g->G_BENDING_EN );
	mutex_unlock(&snt8100fsr_g->ap_lock);
}
