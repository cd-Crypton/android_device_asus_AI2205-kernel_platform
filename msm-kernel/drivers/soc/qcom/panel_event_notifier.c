		 ...
		 struct panel_event_notification *notification)
{
	...
	for (i = 0; i < PANEL_EVENT_NOTIFIER_CLIENT_MAX; i++) {
		...
		// We skip panel paramter for we have only 1 primary panel, that save code and risk to parsing device tree node for active panel
		if (entry->tag != PANEL_EVENT_NOTIFICATION_PRIMARY &&
