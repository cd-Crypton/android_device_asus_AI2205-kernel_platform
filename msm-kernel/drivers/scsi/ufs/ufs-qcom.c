...
#define ASUS_UFS_CHECK 1

#ifdef CONFIG_SCSI_UFS_ASUS
#include "ufs-qcom_asus.h"

#if ASUS_UFS_CHECK
uint asus_ufs_uic_check = 0; /* /sys/module/ufs_qcom/parameters/asus_ufs_uic_check, default is N */
module_param(asus_ufs_uic_check, uint, 0644);
MODULE_PARM_DESC(asus_ufs_uic_check, "");
#endif
#endif
...
#if ASUS_UFS_CHECK
	if ((min_dev_gear == UFS_HS_G4)&&(min_qcom_gear == UFS_HS_G5))
		min_dev_gear = UFS_HS_G5;
#endif
	/*
...
#if ASUS_UFS_CHECK
//printk("%s: is_dev_sup_hs(%d) is_qcom_max_hs(%d) min_qcom_gear(%d) min_dev_gear(%d) gear(%d)\n",
//__func__, is_dev_sup_hs, is_qcom_max_hs, min_qcom_gear, min_dev_gear, agreed_pwr->gear_rx);
#endif
...
#if ASUS_UFS_CHECK
/*
if 	(dev_max_params != NULL)
printk("%s: (%d) max g:[%u %u] l:[%u %u]  pwr:[%u %u] r:%u\n", __func__, status,
    dev_max_params->gear_rx,
	dev_max_params->gear_tx,
	dev_max_params->lane_rx,
	dev_max_params->lane_tx,
	dev_max_params->pwr_rx,
	dev_max_params->pwr_tx,
	dev_max_params->hs_rate
	);
else
printk("%s: (%d) dev_max_params is null\n",__func__,status);
*/
#if 0
if((dev_req_params != NULL)&&(status))
printk("%s: (%d) req g:[%u %u] l:[%u %u]  pwr:[%u %u] r:%u\n", __func__, status,
...
else if(dev_req_params == NULL)
printk("%s: (%d) dev_req_params is null\n",__func__,status);
#endif
#endif
	switch (status) {
	case PRE_CHANGE:
...
#ifdef CONFIG_SCSI_UFS_ASUS
	mutex_init(&host->ufs_info.asus_ufs_lock);
...
if(hba->saved_uic_err){
	asus_ufs_uic_check++;
	printk("asus_ufs_uic_check: 0x%x (%d)\n", hba->saved_uic_err, asus_ufs_uic_check);
    //host->limit_tx_hs_gear = UFS_HS_G4;
    //host->limit_rx_hs_gear = UFS_HS_G4;
}
#endif
...
#if ASUS_UFS_CHECK
static ssize_t asus_ufs_lock_store(struct device *dev,
			struct device_attribute *attr,
			const char *buf, size_t count)
{
	struct ufs_hba *hba = dev_get_drvdata(dev);
	...
	//printk("asus_ufs_lock: (0x%x) %d\n",dev_info->wspecversion, v);
	if (dev_info->wspecversion < 0x0400) {
		//printk("[UFS] cannot dynamic lock\n");
	}else if (v) {
	host->limit_tx_hs_gear = UFS_HS_G4;
	host->limit_rx_hs_gear = UFS_HS_G4;
	} else {
	host->limit_tx_hs_gear = UFS_HS_G5;
	host->limit_rx_hs_gear = UFS_HS_G5;
	}
	return count;
}

static ssize_t asus_ufs_lock_show(struct device *dev,
			struct device_attribute *attr, char *buf)
{
	struct ufs_hba *hba = dev_get_drvdata(dev);
	struct ufs_qcom_host *host = ufshcd_get_variant(hba);

	return scnprintf(buf, PAGE_SIZE, "%d\n", host->limit_rx_hs_gear);
}
static DEVICE_ATTR_RW(asus_ufs_lock);
#endif
...
#ifdef CONFIG_SCSI_UFS_ASUS
	asus_ufshcd_system_suspend(dev);
