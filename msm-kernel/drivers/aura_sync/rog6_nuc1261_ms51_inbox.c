		 ...
		 struct panel_event_notification *notification, void *client_data)
{
	...
	printk("[ROG6_INBOX] Notification type:%d, early_trigger:%d",
	...
	case DRM_PANEL_EVENT_UNBLANK:
		printk("[ROG6_INBOX] DRM_PANEL_EVENT_UNBLANK\n");
		//set_HDC2010_INT_TH(0);
		break;
	case DRM_PANEL_EVENT_BLANK:
		printk("[ROG6_INBOX] DRM_PANEL_EVENT_BLANK\n");
		//set_HDC2010_INT_TH(1);
		break;
	case DRM_PANEL_EVENT_BLANK_LP:
		printk("[ROG6_INBOX] DRM_PANEL_EVENT_BLANK_LP\n");
		break;
	case DRM_PANEL_EVENT_FPS_CHANGE:
		printk("[ROG6_INBOX] DRM_PANEL_EVENT_FPS_CHANGE\n");
		break;
	default:
		printk("[ROG6_INBOX] notification serviced\n");
		break;
	}
}
static void rog_inbox_register_for_panel_events(struct inbox_drvdata *drvdata)
{
	...
	cookie = panel_event_notifier_register(PANEL_EVENT_NOTIFICATION_PRIMARY,
			PANEL_EVENT_NOTIFIER_CLIENT_ROG_INBOX6, 0 /*active_panel*/,
			&rog_inbox_panel_notifier_callback, drvdata);
	if (!cookie) {
		pr_err("[ROG6_INBOX] Failed to register for panel events\n");
		return;
	}

	printk("[ROG6_INBOX] registered for panel notifications --\n");
...
static ssize_t temperature_show(struct device *dev, struct device_attribute *attr,char *buf)
{
	...
	if (g_thermometer == THERMOMETER_SHT4x) {
		...
		if(report_the_last_temp && g_last_temperature != -255){
			printk("[ROG6_INBOX][%s] return last temperature %d\n",__func__ ,g_last_temperature);
			temperature = g_last_temperature;
		}else if(report_the_last_temp && g_last_temperature == -255){
			printk("[ROG6_INBOX][%s] return fake temperature\n",__func__);
			temperature = 35;
		}else{
...
// Set global variable
	...
	hid_err(hdev, "%s: [ROG6_INBOX] rog_inbox_register_for_panel_events +++\n", __func__);
...
static void rog6_inbox_usb_remove(struct hid_device *hdev)
{
	...
	if (drvdata->notifier_cookie){
		printk("[ROG6_INBOX] %s panel_event_notifier_unregister\n",__func__);
