...
struct gpio_control_mux {
	u32 MUX1_OE;	//SOC_GPIO_91
	u32 MUX1_SEL;	//GND
	u32 MUX2_OE;	//BHS_GPIO_07
	u32 MUX2_SEL;	//SOC_GPIO_124
	u32 MUX3_OE;	//SOC_GPIO_53
	u32 MUX3_SEL;	//SOC_GPIO_54
};
...
extern void repeater_set_current_path(enum usb_path path);
int current_path = 0;

static const char * const usb_path_name[] = {
	[PATH_NONE]		= "NONE",
	[PATH_SIDE]		= "SIDE",
	[PATH_BTM]		= "BTM",
	[PATH_HUB]		= "HUB",
...
enum usb3803_mode{
	USB_3803_MODE_STANDBY = 0,
	USB_3803_MODE_BYPASS = 1,
	USB_3803_MODE_HUB = 2,
	USB_3803_MODE_HUB_BTM = 3,
};
...
static int mux_init(struct dwc3_msm *mdwc)
{
	...
	mux_gpio_ctrl->MUX3_OE = of_get_named_gpio(mdwc->dev->of_node, "MUX3_OE", 0);
	ret = gpio_request(mux_gpio_ctrl->MUX3_OE, "MUX3_OE");
	if (ret)
		pr_err("[USB_mux] failed to request MUX3_OE\n");

	mux_gpio_ctrl->MUX3_SEL = of_get_named_gpio(mdwc->dev->of_node, "MUX3_SEL", 0);
	ret = gpio_request(mux_gpio_ctrl->MUX3_SEL, "MUX3_SEL");
	if (ret)
		pr_err("[USB_mux] failed to request MUX3_SEL\n");

	return 0;
}

#ifdef CONFIG_USB_HUB_USB3803_MODULE
static void usb_path_switch(enum usb_path path){
	int ret = 0;

	pr_info("[USB] %s path: %s -> %s\n", __func__, usb_path_name[current_path] ,usb_path_name[path]);

	if (mux_gpio_ctrl->MUX2_OE == -EPROBE_DEFER) {
		pr_err("[USB_mux] %s MUX2_OE=%d, retry request MUX2_OE\n", __func__, mux_gpio_ctrl->MUX2_OE);
		mux_gpio_ctrl->MUX2_OE = of_get_named_gpio(g_mdwc->dev->of_node, "MUX2_OE", 0);
		ret = gpio_request(mux_gpio_ctrl->MUX2_OE, "MUX2_OE");
		if (ret)
			pr_err("[USB_mux] %s failed to request MUX2_OE\n", __func__);
		pr_info("[USB_mux] %s MUX2_OE=%d\n", __func__, mux_gpio_ctrl->MUX2_OE);
	}

	if(path == PATH_NONE){
		pr_info("[USB] %s NONE not control mux gpio\n", __func__);
		/*
		pr_info("[USB] %s NONE mux_gpio=0 GND 1 0 0 0\n", __func__);
		ret = gpio_direction_output(mux_gpio_ctrl->MUX1_OE, 0);//mux1 disable(reverse)
		if (ret)
			pr_err("[USB_mux] failed to control MUX1_OE\n");
		ret = gpio_direction_output(mux_gpio_ctrl->MUX2_OE, 1);//mux2 disable
		if (ret)
			pr_err("[USB_mux] failed to control MUX2_OE\n");
		ret = gpio_direction_output(mux_gpio_ctrl->MUX2_SEL, 0);//x
		if (ret)
			pr_err("[USB_mux] failed to control MUX2_SEL\n");
		ret = gpio_direction_output(mux_gpio_ctrl->MUX3_OE, 0);//mux3 disable(reverse)
		if (ret)
			pr_err("[USB_mux] failed to control MUX3_OE\n");
		ret = gpio_direction_output(mux_gpio_ctrl->MUX3_SEL, 0);//x
		if (ret)
			pr_err("[USB_mux] failed to control MUX3_SEL\n");
		*/
		usb3803_set_mode(USB_3803_MODE_STANDBY);
	} else if (path == PATH_SIDE){
		pr_info("[USB] %s SIDE mux_gpio=0 GND 0 0 0 0\n", __func__);
		ret = gpio_direction_output(mux_gpio_ctrl->MUX1_OE, 0);//mux1 disable(reverse)
		if (ret)
			pr_err("[USB_mux] failed to control MUX1_OE\n");
		ret = gpio_direction_output(mux_gpio_ctrl->MUX2_OE, 0);//mux2 enable
		if (ret)
			pr_err("[USB_mux] failed to control MUX2_OE\n");
		ret = gpio_direction_output(mux_gpio_ctrl->MUX2_SEL, 0);//mux2 sel to side
		if (ret)
			pr_err("[USB_mux] failed to control MUX2_SEL\n");
		ret = gpio_direction_output(mux_gpio_ctrl->MUX3_OE, 0);//mux3 disable(reverse)
		if (ret)
		...
		if (ret)
			pr_err("[USB_mux] failed to control MUX3_SEL\n");
		usb3803_set_mode(USB_3803_MODE_STANDBY);
	} else if (path == PATH_BTM){
		pr_info("[USB] %s BTM mux_gpio=0 GND 1 0 1 0\n", __func__);
		ret = gpio_direction_output(mux_gpio_ctrl->MUX1_OE, 0);//mux1 disable(reverse)
		if (ret)
			pr_err("[USB_mux] failed to control MUX1_OE\n");
		ret = gpio_direction_output(mux_gpio_ctrl->MUX2_OE, 1);//mux2 disable
		...
		if (ret)
			pr_err("[USB_mux] failed to control MUX2_SEL\n");
		ret = gpio_direction_output(mux_gpio_ctrl->MUX3_OE, 1);//mux3 enable(reverse)
		if (ret)
			pr_err("[USB_mux] failed to control MUX3_OE\n");
		ret = gpio_direction_output(mux_gpio_ctrl->MUX3_SEL, 0);//mux3 sel to btm
		if (ret)
			pr_err("[USB_mux] failed to control MUX3_SEL\n");
		usb3803_set_mode(USB_3803_MODE_STANDBY);
	} else if (path == PATH_HUB){
		pr_info("[USB] %s HUB mux_gpio=1 GND 0 1 1 1\n", __func__);
		ret = gpio_direction_output(mux_gpio_ctrl->MUX1_OE, 1);//mux1 enable(reverse)
		if (ret)
			pr_err("[USB_mux] failed to control MUX1_OE\n");
		ret = gpio_direction_output(mux_gpio_ctrl->MUX2_OE, 0);//mux2 enable
		if (ret)
			pr_err("[USB_mux] failed to control MUX2_OE\n");
		ret = gpio_direction_output(mux_gpio_ctrl->MUX2_SEL, 1);//mux2 sel to hub
		if (ret)
			pr_err("[USB_mux] failed to control MUX2_SEL\n");
		ret = gpio_direction_output(mux_gpio_ctrl->MUX3_OE, 1);//mux3 enable(reverse)
		if (ret)
			pr_err("[USB_mux] failed to control MUX3_OE\n");
		ret = gpio_direction_output(mux_gpio_ctrl->MUX3_SEL, 1);//mux3 sel to hub
		if (ret)
			pr_err("[USB_mux] failed to control MUX3_SEL\n");
	...
	current_path = path;
	repeater_set_current_path(current_path);
	pr_info("[USB] %s current_path=%s\n", __func__, usb_path_name[current_path]);
	return;
}

static void update_usb_path(enum usb_source source, enum usb_role role){
	struct dwc3_msm *mdwc = g_mdwc;
	enum usb_path pre_path;

	pre_path = current_path;

	role_table[source] = role;

	pr_info("[USB] %s role_table=%d %d %d %d\n",__func__, role_table[0], role_table[1], role_table[2], role_table[3]);

	if (role_table[INBOX_DET] == USB_ROLE_HOST || role_table[GAMEPAD_DET] == USB_ROLE_HOST){
		if (current_path != PATH_HUB)
			usb_path_switch(PATH_HUB);
	} else if (role_table[SIDE_CC] == USB_ROLE_HOST || role_table[SIDE_CC] == USB_ROLE_DEVICE){
		if (current_path != PATH_SIDE)
			usb_path_switch(PATH_SIDE);
	} else if (role_table[BTM_CC] == USB_ROLE_HOST || role_table[BTM_CC] == USB_ROLE_DEVICE){
		if (current_path != PATH_BTM)
			usb_path_switch(PATH_BTM);
	} else {
		if (current_path != PATH_NONE)
			usb_path_switch(PATH_NONE);
	}

	if (role == USB_ROLE_HOST && pre_path != PATH_NONE && pre_path != current_path){
		dev_info(mdwc->dev, "[USB] %s path %s->%s, restart host\n", __func__, usb_path_name[pre_path], usb_path_name[current_path]);
		dwc3_start_stop_host(mdwc, false);
		dwc3_start_stop_host(mdwc, true);
	}
	if (role == USB_ROLE_DEVICE && pre_path != PATH_NONE && pre_path != current_path){
		dev_info(mdwc->dev, "[USB] %s path %s->%s, restart device\n", __func__, usb_path_name[pre_path], usb_path_name[current_path]);
...
static void restore_role(void){
	struct dwc3_msm *mdwc = g_mdwc;

	if(role_table[SIDE_CC] == USB_ROLE_DEVICE || role_table[BTM_CC] == USB_ROLE_DEVICE){
		pr_info("[USB] %s DEVICE\n",__func__);
		mdwc->vbus_active = true;
		mdwc->id_state = DWC3_ID_FLOAT;
		dev_info(mdwc->dev, "[USB] %s current_path:%s, restart device\n", __func__, usb_path_name[current_path]);
		dwc3_start_stop_device(mdwc, false);
		dwc3_start_stop_device(mdwc, true);
	} else if (role_table[SIDE_CC] == USB_ROLE_HOST || role_table[BTM_CC] == USB_ROLE_HOST){
		pr_info("[USB] %s HOST\n",__func__);
		mdwc->vbus_active = false;
		mdwc->id_state = DWC3_ID_GROUND;
		dev_info(mdwc->dev, "[USB] %s current_path:%s, restart host\n", __func__, usb_path_name[current_path]);
		dwc3_start_stop_host(mdwc, false);
		dwc3_start_stop_host(mdwc, true);
	}
...
void battery_role_switch(bool on)
{
	...
	enum usb_role switch_role = USB_ROLE_NONE;
	...
	dev_info(mdwc->dev, "[USB] %s cur_role:%s new_role:%d\n", __func__, usb_role_string(cur_role), on);

#ifdef CONFIG_USB_HUB_USB3803_MODULE
	if (on)
		switch_role = USB_ROLE_DEVICE;
	else
		switch_role = USB_ROLE_NONE;

	update_usb_path(BTM_CC, switch_role);
	if (current_path == PATH_HUB || current_path == PATH_SIDE){
		dev_info(mdwc->dev, "[USB] %s current_path:%s, ignore role switch\n", __func__, usb_path_name[current_path]);
	 ...
	 * PHYs.
	 */
	if (mdwc->drd_state != DRD_STATE_UNDEFINED && cur_role == switch_role) {
...
/* Notify charger OTG5V and USB HOST */
void rt1715_role_switch(enum usb_role role)
{
	...
	if (!mdwc) {
		pr_err("%s dwc3 probe not completed\n");
		return;
	}
...
#ifdef CONFIG_USB_HUB_USB3803_MODULE
	update_usb_path(INBOX_DET, enable);//enable 1 => USB_ROLE_HOST, 0 => USB_ROLE_NONE
#endif
	if(enable){
		mdwc->vbus_active = false;
		mdwc->id_state = DWC3_ID_GROUND;
...
void Gamepad_role_switch(int enable) {
...
#ifdef CONFIG_USB_HUB_USB3803_MODULE
	update_usb_path(GAMEPAD_DET, enable);//enable 1 => USB_ROLE_HOST, 0 => USB_ROLE_NONE
#endif
	if(enable){
		mdwc->vbus_active = false;
		mdwc->id_state = DWC3_ID_GROUND;
...
EXPORT_SYMBOL_GPL(Gamepad_role_switch);
	...
	struct device_attribute *attr, char *buf)
{
	...
	else if (current_path==PATH_SIDE)
		return sprintf(buf, "%s", "SIDE\n");
	else if (current_path==PATH_BTM)
		return sprintf(buf, "%s", "BTM\n");
	else if (current_path==PATH_HUB)
...
static ssize_t Inbox_enable_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	if(role_table[INBOX_DET] == USB_ROLE_HOST)
...
static ssize_t Gamepad_enable_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	if(role_table[GAMEPAD_DET] == USB_ROLE_HOST)
		return sprintf(buf, "%s", "1\n");
	else
		return sprintf(buf, "%s", "0\n");
}

static ssize_t Gamepad_enable_store(
	struct device *dev, struct device_attribute *attr,
	const char *buf, size_t size)
{
	struct dwc3_msm *mdwc = dev_get_drvdata(dev);
	...
	dev_info(mdwc->dev, "[USB] %s: %s\n", __func__, buf);
	if (!strncmp(buf, "1", 1)) {
		Gamepad_role_switch(1);
	} else if (!strncmp(buf, "0", 1)) {
		Gamepad_role_switch(0);
	}

	return size;
}
static DEVICE_ATTR_RW(Gamepad_enable);

static ssize_t rt1715_mode_show(struct device *dev, struct device_attribute *attr,
		char *buf)
{
	enum usb_role cur_role;

	cur_role = role_table[BTM_CC];
	if (cur_role == USB_ROLE_DEVICE)
		return scnprintf(buf, PAGE_SIZE, "peripheral\n");
	if (cur_role == USB_ROLE_HOST)
		return scnprintf(buf, PAGE_SIZE, "host\n");
...
static ssize_t rt1715_mode_store(struct device *dev, struct device_attribute *attr,
		const char *buf, size_t count)
{
	struct dwc3_msm *mdwc = dev_get_drvdata(dev);
	enum usb_role role = USB_ROLE_NONE;
	...
	dbg_log_string("mode_request:%s\n", usb_role_string(role));
	rt1715_role_switch(role);

	return count;
}
static DEVICE_ATTR_RW(rt1715_mode);
#endif
...
#ifdef CONFIG_USB_HUB_USB3803_MODULE
	&dev_attr_usb_path.attr,
	&dev_attr_Inbox_enable.attr,
	&dev_attr_Gamepad_enable.attr,
	&dev_attr_rt1715_mode.attr,
#endif
...
int dwc3_msm_set_dp_mode(struct device *dev, bool dp_connected, int lanes)
{
	...
	dev_info(mdwc->dev, "[USB] %s DP: cur_state:%d new_state:%d lanes:%d\n", __func__,
				mdwc->dp_state, dp_connected, lanes);
#ifdef CONFIG_USB_HUB_USB3803_MODULE
	if (current_path == PATH_HUB){
		dev_info(mdwc->dev, "[USB] %s, in HUB mode, skip!\n", __func__);
		return 0;
	}
#endif
	...
	if (!dp_connected) {
		dev_info(mdwc->dev, "[USB] %s DP disconnected, refcnt:%d\n", __func__, mdwc->refcnt_dp_usb);
	...
	dev_info(mdwc->dev, "[USB] %s Set DP lanes:%d refcnt:%d\n", __func__ , lanes, mdwc->refcnt_dp_usb);
...
static int dwc3_msm_probe(struct platform_device *pdev)
{
	...
	mux_init(mdwc);
	if (mux_gpio_ctrl->MUX2_OE == -EPROBE_DEFER) {
		dev_err(&pdev->dev, "[USB_mux] MUX2_OE=%d, usb_path_switch will retry request\n", mux_gpio_ctrl->MUX2_OE);
	}
...
static int dwc3_otg_start_host(struct dwc3_msm *mdwc, int on)
{
	...
	if (on) {
		...
		dev_info(mdwc->dev, "[USB] %s: max_speed=%s\n", __func__, dwc->maximum_speed);
