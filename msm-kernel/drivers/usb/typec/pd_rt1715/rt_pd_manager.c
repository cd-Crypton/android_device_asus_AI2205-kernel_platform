...
extern void Gamepad_role_switch(int enable);
uint16_t vid=0;
extern uint8_t gamepad_active;
...
/*
 * Send extcon message to IMS that notify asus adapter plugin or unplug.
*/
void rt1715_vid_notify(void) {

	if (quickchg_extcon == NULL || g_IsBootComplete == false){
...
static inline void stop_usb_peripheral(struct rt_pd_manager_data *rpmd)
{
	...
	/*DR swap enable/disable host & client*/
	rt1715_role_switch(USB_ROLE_NONE);
}
				...
				unsigned long event, void *data)
{
	...
	case TCP_NOTIFY_SOURCE_VBUS:
		...
		//if (noti->vbus_state.mv) {
			//if (!rpmd->vbus_reg_en)
				//ret = regulator_enable(rpmd->vbus_reg);
		//} else {
			//if (rpmd->vbus_reg_en)
				//ret = regulator_disable(rpmd->vbus_reg);
		//}
		//if (ret)
			//dev_err(rpmd->dev, "%s source vbus fail(%d)\n",
					   //__func__, ret);
		//else
			//rpmd->vbus_reg_en = !!noti->vbus_state.mv;
		...
		/* Notify charger OTG5V and USB HOST */
		enable = (noti->vbus_state.mv > 0) ? true : false;

		ret = BTM_OTG_EN(enable);
		if (ret) {
			dev_err(rpmd->dev, "%s failed to control GPIO BTM_OTG_EN & HAPTICS_BOOST_VREG_EN\n", __func__);
		}
		break;
	case TCP_NOTIFY_TYPEC_STATE:
		...
		if (old_state == TYPEC_UNATTACHED &&
			...
			if (new_state == TYPEC_ATTACHED_CUSTOM_SRC) {
				dev_info(rpmd->dev, "%s tcpc_typec_state = TYPEC_ATTACHED_CUSTOM_SRC\n", __func__);
			...
			if (old_state == TYPEC_ATTACHED_CUSTOM_SRC) {
				dev_info(rpmd->dev, "%s tcpc_pre_typec_state = TYPEC_ATTACHED_CUSTOM_SRC\n", __func__);
	...
	case TCP_NOTIFY_PD_STATE:
		...
		tcpc_pd_state = noti->pd_state.connected;
		dev_info(rpmd->dev, "%s pd state = %d\n",
				    __func__, rpmd->pd_connect_state);
		switch (rpmd->pd_connect_state) {
		case PD_CONNECT_NONE:
			dev_info(rpmd->dev, "%s tcpc_pd_state = PD_CONNECT_NONE\n", __func__);
			vid_ext = 0;
			if (gamepad_active) {
				gamepad_active = 0;
				dev_info(rpmd->dev, "%s gamepad_active = %d\n", __func__, gamepad_active);
			}
			rt1715_pdo_notify();
			rt1715_vid_notify();
			break;
		case PD_CONNECT_HARD_RESET:
			dev_info(rpmd->dev, "%s tcpc_pd_state = PD_CONNECT_HARD_RESET,\n", __func__);
			rt1715_pdo_notify();
			break;
		case PD_CONNECT_PE_READY_SNK_APDO:
			...
			old_state = noti->typec_state.old_state;
			peer_usb_comm = (ret &= DPM_FLAGS_PARTNER_USB_COMM);
			dev_info(rpmd->dev,"%s tcpc_pd_state = PD_CONNECT_PE_READY_SNK_APDO, USB_COMM = %d\n", __func__, peer_usb_comm);
			rt1715_pdo_notify();
			rt1715_vid_notify();
			if (peer_usb_comm && old_state == TYPEC_UNATTACHED) {
				rt1715_role_switch(USB_ROLE_DEVICE);
			}
		...
		case PD_CONNECT_PE_READY_SNK_PD30:
			...
			//smblib_set_prop(rpmd,
				//POWER_SUPPLY_PROP_PD_USB_SUSPEND_SUPPORTED,
					//&val);
			if(rpmd->pd_connect_state == PD_CONNECT_PE_READY_SNK 
				|| rpmd->pd_connect_state == PD_CONNECT_PE_READY_SNK_PD30) {
					old_state = noti->typec_state.old_state;
					peer_usb_comm = (ret &= DPM_FLAGS_PARTNER_USB_COMM);
					dev_info(rpmd->dev, "%s tcpc_pd_state = PD_CONNECT_PE_READY_SNK/PD3.0, USB_COMM = %d\n", __func__, peer_usb_comm);
					rt1715_pdo_notify();
					rt1715_vid_notify();
					if (peer_usb_comm && old_state == TYPEC_UNATTACHED) {
						rt1715_role_switch(USB_ROLE_DEVICE);
		...
		case PD_CONNECT_PE_READY_SRC_PD30:
			...
			if(rpmd->pd_connect_state == PD_CONNECT_PE_READY_SRC 
				|| rpmd->pd_connect_state == PD_CONNECT_PE_READY_SRC_PD30) {
					vid = partner_vdos[0] & 0xFFFF;
					dev_info(rpmd->dev, "%s tcpc_pd_state = PD_CONNECT_PE_READY_SRC/PD3.0\n", __func__);
					if (vid == 0x0b05){
						gamepad_active = 1;
						dev_info(rpmd->dev, "%s gamepad_active = %d\n", __func__, gamepad_active);
						/* enable GamePad hub mode */
...
static int __tcpc_class_complete_work(struct device *dev, void *data)
{
	...
	if (tcpc != NULL) {
		...
		if (ret < 0) {
			printk("%s: register bat notifier fail\n", __func__);
			return -EINVAL;
		}
#endif /* CONFIG_RECV_BAT_ABSENT_NOTIFY */
#endif /* CONFIG_USB_POWER_DELIVERY */
...
//#endif /* (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 4, 0)) */
	...
	//rpmd->vbus_reg = devm_regulator_get(rpmd->dev, "vbus");
	//if (IS_ERR(rpmd->vbus_reg)) {
		//ret = PTR_ERR(rpmd->vbus_reg);
		//dev_err(rpmd->dev, "%s get vbus reg fail(%d)\n", __func__, ret);
		//ret = -EPROBE_DEFER;
		//if (probe_cnt >= PROBE_CNT_MAX)
			//goto out;
		//else
			//goto err_get_vbus_reg;
	//}
	//rpmd->vbus_reg_en = false;
...
//#endif /* (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 4, 0)) */
	...
	if (!rpmd->tcpc) {
		...
		if (probe_cnt >= PROBE_CNT_MAX)
			...
			// sleep 1 sec for getting tcpc device
			msleep(1000);
			goto err_get_tcpc_dev;
		}
	}

	// Check rt1711_i2c_probe is completed
	printk("%s irq_enabled = %d\n", __func__, rpmd->tcpc->irq_enabled);
	if (!rpmd->tcpc->irq_enabled) {
		dev_err(rpmd->dev, "%s rt1711_i2c_probe is not completed yet\n", __func__);
		ret = -EPROBE_DEFER;
		if (probe_cnt >= PROBE_CNT_MAX)
			goto out;
		else {
			// sleep 1 sec for waiting irq enable ready
			msleep(1000);
			goto err_init_typec;
		}
	}
