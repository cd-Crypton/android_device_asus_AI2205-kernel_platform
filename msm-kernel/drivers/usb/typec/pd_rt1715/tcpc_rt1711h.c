			...
			u8 reg, int len, void *data)
{
	...
	if (ret < 0)
		dev_err(chip->dev, "block read fail (%d)\n", ret);
	return ret;
}

static int rt1711_block_write(struct i2c_client *i2c,
			u8 reg, int len, const void *data)
{
	struct rt1711_chip *chip = i2c_get_clientdata(i2c);
	int ret = 0;
	...
	if (ret < 0)
		dev_err(chip->dev, "block write fail(%d)\n", ret);
	return ret;
}
	...
	struct tcpc_device *tcpc, u8 reg, const u8 data)
{
	...
	if (ret < 0)
		dev_err(chip->dev, "write8 fail(%d)\n", ret);
	return ret;
}

static inline int rt1711_i2c_write16(
		struct tcpc_device *tcpc, u8 reg, u16 data)
{
	struct rt1711_chip *chip = tcpc_get_dev_data(tcpc);
	int ret = 0;

	data = cpu_to_le16(data);
	ret = regmap_bulk_write(chip->regmap, reg, &data, 2);
	if (ret < 0)
		dev_err(chip->dev, "write16 fail(%d)\n", ret);
	return ret;
}
...
static inline int rt1711_i2c_read16(struct rt1711_chip *chip, u8 reg)
{
	u16 data;
	int ret;
	...
	if (ret < 0) {
		dev_err(chip->dev, "read16 fail(%d)\n", ret);
		return ret;
	}
...
static int rt1711_init_vbus_cal(struct tcpc_device *tcpc)
{
	...
	ret = rt1711_i2c_read8(tcpc, RT1711H_REG_EFUSE5);
	if (ret < 0)
		goto out;
	...
	ret = rt1711_i2c_write8(tcpc, RT1711H_REG_EFUSE5, data);
out:
	ret = rt1711_block_write(chip->client, RT1711H_REG_UNLOCK_PW_2,
...
static int rt1711_init_alert_mask(struct tcpc_device *tcpc)
{
	...
	return rt1711_i2c_write16(tcpc, TCPC_V10_REG_ALERT_MASK, mask);
}
...
static int rt1711_init_alert(struct tcpc_device *tcpc)
{
	...
	ret = rt1711_i2c_write16(tcpc, TCPC_V10_REG_ALERT_MASK, 0);
	if (ret)
		return ret;
	ret = rt1711_i2c_write16(tcpc, TCPC_V10_REG_ALERT, 0xffff);
	if (ret)
		return ret;
	...
	rt1711_pinctrl_state_active = pinctrl_lookup_state(rt1711_pinctrl, "rt1711h_int_n");
...
int rt1711_get_alert_mask(struct tcpc_device *tcpc, uint32_t *mask)
{
	...
	ret = rt1711_i2c_read16(chip, TCPC_V10_REG_ALERT_MASK);
	if (ret < 0)
		return ret;
...
static inline int rt1711h_check_revision(struct rt1711_chip *chip)
{
	u16 vid, pid, did;
	int ret = 0;

	vid = rt1711_i2c_read16(chip, TCPC_V10_REG_VID);
	if (vid < 0) {
		dev_err(chip->dev, "read chip ID fail(%d)\n", vid);
		return -EIO;
	}
	...
	pid = rt1711_i2c_read16(chip, TCPC_V10_REG_PID);
	if (pid < 0) {
		dev_err(chip->dev, "read product ID fail(%d)\n", pid);
		return -EIO;
	}
	...
	ret = regmap_write(chip->regmap, RT1711H_REG_SWRESET, 1);
	if (ret < 0)
		return ret;

	usleep_range(1000, 2000);

	did = rt1711_i2c_read16(chip, TCPC_V10_REG_DID);
	if (did < 0) {
		dev_err(chip->dev, "read device ID fail(%d)\n", did);
		return -EIO;
	}
...
static const struct regmap_config rt1711_regmap_config = {
	.reg_bits = 8,
	.val_bits = 8,
	.max_register = RT1711H_REG_EFUSE5,
};
				...
				const struct i2c_device_id *id)
{
	...
	chip->regmap = devm_regmap_init_i2c(client, &rt1711_regmap_config);
	...
	chip_id = rt1711h_check_revision(chip);
	if (chip_id < 0)
		return chip_id;
