...
int expected_mode = USB_3803_MODE_STANDBY;
int current_clk;
...
int usb3803_set_mode(int mode)
{
	...
	expected_mode = mode;
	if(!hub_usb3803){
		pr_err("%s hub_usb3803 not ready!\n", __func__);
		return 0;
	}
	...
	case USB_3803_MODE_HUB:
		...
		//check port1 & port2
		usb3803_register_read(PDS, &data);
		pr_info("[%s] default PDS: 0x%x \n", __func__, data);

		//set tuning eye diagram param
		usb3803_register_write(BST21, 0x55);
		usb3803_register_write(BSTUP3, 0x35);
	...
	case USB_3803_MODE_HUB_BTM:
		pr_info("%s USB_3803_MODE_HUB_BTM\n", __func__);
		usb3803_reset_n(0);
		msleep(20);
		/* enable clock */
		usb3803_clock_en(1);
		...
		usb3803_register_read(PDS, &data);
		pr_info("[%s] default PDS: 0x%x \n", __func__, data);
		data |= 0x06;//bit1: port1 disable, bit2: port2 disable
		usb3803_register_write(PDS, data);
		usb3803_register_read(PDS, &data);
		pr_info("[%s] disable port1 & port2, PDS: 0x%x \n", __func__, data);

		//set tuning eye diagram param
		usb3803_register_write(BST21, 0x75);
		usb3803_register_write(BSTUP3, 0x34);
...
int usb3803_clock_en(int val)
{
	...
	if (val){
		pr_info("%s enable ref_clk\n", __func__);
		err = clk_prepare_enable(ref_clk);
		if (err) {
			pr_err("%s fail to enable ref_clk\n", __func__);
			return err;
		}
		current_clk = val;
	} else {
		if (current_clk) {
			pr_info("%s disable ref_clk\n", __func__);
			clk_disable_unprepare(ref_clk);
			current_clk = val;
		} else {
			pr_info("%s ref_clk already disable\n", __func__);
		}
	}
...
int usb3803_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	//ref_clk init
	ref_clk = devm_clk_get(hub->dev, "hub_ref_clk");
	if (IS_ERR(ref_clk)){
		pr_err("%s clk_get ref_clk fail!\n", __func__);
		ref_clk = NULL;
	}
	current_clk = 0;
	usb3803_connect_n(1);
	...
	pr_info("%s expected_mode=%d\n", __func__, expected_mode);
	usb3803_set_mode(expected_mode);
