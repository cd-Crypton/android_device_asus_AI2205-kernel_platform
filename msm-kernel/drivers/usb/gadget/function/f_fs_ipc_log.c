...
struct work_struct acc_full_work;
struct work_struct acc_reset_work;

char *envs_reset[2] = { "F_ACC_SPEED=RESET", NULL };
char *envs_full[2] = { "F_ACC_SPEED=FULL", NULL };

static void full_uevent_work(struct work_struct *w)
{
	if (!g_acc_device)
		return;
...
static void reset_uevent_work(struct work_struct *w)
{
	if (!g_acc_device)
		return;
				   ...
				   struct pt_regs *regs)
{
	...
	queue_work(acc_wq, &acc_reset_work);
	pr_info("kprobe %s\n", __func__);
				   ...
				   struct pt_regs *regs)
{
	...
	if (!strcmp(dev->manufacturer, "Android") && !strcmp(dev->version, "1.0")) {
		queue_work(acc_wq, &acc_full_work);
	}
	return 0;
}
...
exit_misc_register(struct kretprobe_instance *ri,
	...
	if(!strcmp(msic_device->name, "usb_accessory")) {
		...
		pr_info("kprobe %s\n", __func__);
		INIT_WORK(&acc_full_work, full_uevent_work);
		INIT_WORK(&acc_reset_work, reset_uevent_work);
	}
...
static int __init kretprobe_init(void)
{
	...
	acc_wq = alloc_ordered_workqueue("acc_wq", 0);
	if (!acc_wq) {
		pr_err("%s: Unable to create workqueue acc_wq\n", __func__);
		return -ENOMEM;
	}
